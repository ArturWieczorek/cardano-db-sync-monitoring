...
nohup python3 scripts/db-sync-resource-monitor.py \
  --env preprod --db-sync-ver 13.6.0.5 --pg-dbname preprod_13.6.0.5 \
  > logs/db-sync-resource-monitor-13.6.0.5.log 2>&1 &
...
nohup python3 scripts/db-sync-resource-monitor.py \
  ...
  > logs/db-sync-resource-monitor-13.7.1.0.log 2>&1 &
...
python3 scripts/db-sync-epoch-report.py --pg-dbname preprod_13.6.0.5,preprod_13.7.1.0
```
...
Same flow works for cardano-node A/B comparisons via `node-resource-monitor.py` / `node-plot.py` - node-side has no `db-sync-epoch-report.py` equivalent because it doesn't write to postgres.
...
python3 scripts/node-resource-monitor.py --env preprod --node-ver 11.0.1

# Background, survives SSH/tmux disconnects
nohup python3 scripts/node-resource-monitor.py --env preprod --node-ver 11.0.1 \
  > logs/node-resource-monitor-preprod.log 2>&1 &

# JSON-per-line output for log analyzers (jq / fluentd / etc.)
python3 scripts/node-resource-monitor.py --env preprod --node-ver 11.0.1 --json
```

`--node-ver` is **your** label; it tags rows in `data/cardano-node/<env>.db` so multiple versions can coexist. Full reference: [`# node-resource-monitor.py`](#node-resource-monitorpy).
...
python3 scripts/db-sync-resource-monitor.py \
  --env preprod --db-sync-ver 13.7.1.0 --pg-dbname preprod_13.7.1.0
...
nohup python3 scripts/db-sync-resource-monitor.py \
  --env preprod --db-sync-ver 13.7.1.0 --pg-dbname preprod_13.7.1.0 \
  > logs/db-sync-resource-monitor-preprod.log 2>&1 &
...
Each version-under-test gets its own `--pg-dbname` AND a distinct `--db-sync-ver` label. Same env (e.g. `preprod`) means everything lands in the same SQLite stats DB (`data/cardano-db-sync/preprod.db`), where the version labels keep runs separated. Full reference: [`# db-sync-resource-monitor.py`](#db-sync-resource-monitorpy).
...
Standalone scraper that polls the node's Prometheus endpoint for GHC runtime gauges (GC, allocations, heap/live bytes, mempool) and appends them to the same `<env>.db` under the same `--node-ver` label. Run alongside `node-resource-monitor.py` when you want the runtime signals psutil can't see (e.g. comparing GC pressure or allocation rates across node versions).
...
# On-disk db-directory size (total + lsm subdir) - needs node-db-size-monitor.py data.
# Add --x-axis slot to align two runs by chain position (time axis won't line them up).
python3 scripts/node-plot.py --env preprod --versions 10.1.4,11.0.1 --metrics disk --x-axis slot
...
# On-disk ledger-state size (total + lsm subdir) - needs db-sync-ledger-size-monitor.py data.
# Add --x-axis slot to align two runs by chain position (time axis won't line them up).
python3 scripts/db-sync-plot.py --env preprod --versions 13.6.0.5,13.7.1.0 --metrics disk --x-axis slot
...
```bash
# Single version
python3 scripts/db-sync-epoch-report.py --pg-dbname preprod_13.7.1.0

# A/B comparison - produces a headline-deltas table
python3 scripts/db-sync-epoch-report.py --pg-dbname preprod_13.6.0.5,preprod_13.7.1.0

# Mainnet-safe: skip the expensive per-epoch fetchers (~5-10 min saved)
python3 scripts/db-sync-epoch-report.py --pg-dbname preprod_13.7.1.0 --skip-slow

# Heavy: also compute p95 tx size per epoch (adds 5-20 min on mainnet)
python3 scripts/db-sync-epoch-report.py --pg-dbname preprod_13.7.1.0 --with-p95
```

HTML lands in `plots/cardano-db-sync/`; text reports in `stats/`. Full reference: [`# db-sync-epoch-report.py`](#db-sync-epoch-reportpy).
...
- **`db-sync-epoch-report.py`** - one-shot Postgres report (epoch stats + DB/table sizes), independent of the SQLite stats DB.
...
```sh
python3 scripts/db-sync-resource-monitor.py --env preprod --db-sync-ver 13.6.0.5 --pg-dbname preprod_13.6.0.5_metrics
...
```sh
nohup .venv/bin/python scripts/db-sync-resource-monitor.py --env preprod --db-sync-ver 13.6.0.5 \
  --pg-dbname preprod_13.6.0.5_metrics > logs/db-sync-resource-monitor-preprod.log 2>&1 &
```
...
```bash
$ python3 scripts/db-sync-resource-monitor.py
usage: db-sync-resource-monitor.py [-h] --env {mainnet,preprod,preview} --db-sync-ver DB_SYNC_VER
                          [--pg-host PG_HOST] [--pg-port PG_PORT] [--pg-user PG_USER]
                          --pg-dbname PG_DBNAME [--interval INTERVAL] [--json]
db-sync-resource-monitor.py: error: the following arguments are required: --env, --db-sync-ver, --pg-dbname
```
...
Same idea as `db-sync-resource-monitor.py`, but tracks a running `cardano-node` process. Stats go to `data/cardano-node/<env>.db`. The script attaches by matching the process executable and discovers the node's socket path from its command line.

## Run

```sh
$ python3 scripts/node-resource-monitor.py --env preprod --node-ver 10.1.4
...
- `--json` - emit one JSON object per sample on stdout (instead of the pipe-separated form). Same shape as `db-sync-resource-monitor.py --json`, with `slot_no`, `epoch_no`, `era`, `sync_percent`, `cpu_percent`, `rss_mib`, etc.
...
At startup the monitor reports whether the chosen version label already has samples in the SQLite DB (same notice as `db-sync-resource-monitor.py`), so you can tell at a glance whether you're starting fresh or extending an earlier session.
...
Match the `--node-ver` / `--db-sync-ver` label to the one you gave the matching `*-monitor.py` so the disk series joins the rest of that run's metrics under the same version. If the label you pass is new to the DB while other labels already exist, the collector prints a startup warning listing them - a guard against the typo that would otherwise shard the series under a second label.
...
Both write a `disk_metrics` table (`ts`, `slot_no`, `path`, `total_bytes`, `lsm_bytes`, `version`) in WAL mode, so a size collector can run concurrently with the resource monitor on the same `<env>.db`. `slot_no` is left NULL - the collector deliberately doesn't query the node/db-sync for a slot to stay lightweight. By default disk is plotted against wall-clock time, but `--x-axis slot` derives a slot per sample (nearest-timestamp lookup against the concurrent resource monitor's `memory_metrics`), which is what lets you **compare two runs' disk growth aligned by chain position** - on the time axis they'd sit in disjoint wall-clock windows. Visualize it with `db-sync-plot.py --metrics disk` / `node-plot.py --metrics disk` (below).


# RTS / runtime metrics monitor

`node-rts-monitor.py` is a standalone collector that scrapes the cardano-node Prometheus endpoint for GHC runtime gauges - GC counts, allocations, heap/live bytes, and mempool - and appends them to the same `data/cardano-node/<env>.db` under the same version label `node-resource-monitor.py` uses, so the RTS curves join the rest of that run's metrics by `version`. These are the resource signals a GHC-version or allocator change shows up in, and that psutil's process-level RSS/CPU can't see.

It's a separate process on purpose: it adds an HTTP scrape per interval, isolated here behind a `--timeout` and a `try/except`, so a slow or unreachable endpoint just skips the sample and never stalls or crashes the psutil/tip sampling `node-resource-monitor.py` does. `node-resource-monitor.py` is left untouched - zero risk to a running mainnet collector.
...
Match the `--node-ver` label to the one you gave `node-resource-monitor.py` so the RTS series joins the rest of that run's metrics under the same version. If the label is new to the DB while other labels already exist, the collector warns at startup and lists them - a guard against a typo'd `--node-ver` sharding the series under a second label.
...
The version picker lists the **union of labels across every version-keyed table** (`node_version`, `memory_metrics`, `cpu_metrics`, `node_ingest_metrics`, `disk_metrics`, `rts_metrics`), not just `node_version`. So a run collected only by an optional collector - or saved under a mistyped `--node-ver` - still shows up here instead of being silently invisible. If you see a label you didn't expect (e.g. one matching an env-dir name rather than a node version), that's a typo'd `--node-ver`; merge it with `scripts/rename-version.py`.
...
| `ingest` | Sync time by era (top bar chart) + sync duration per epoch (line). Reads from `node_ingest_metrics` written by the updated `node-resource-monitor.py`. |
...
`--metrics ingest` requires the `node_ingest_metrics` table which the current `node-resource-monitor.py` writes. Older monitor runs that pre-date this table get a warning and are dropped from the plot; their `--metrics cpu_ram` plots continue to work normally.
...
```bash
$ python3 scripts/db-sync-epoch-report.py
usage: db-sync-epoch-report.py [-h] [--pg-host PG_HOST] [--pg-port PG_PORT] [--pg-user PG_USER] --pg-dbname PG_DBNAME [--outdir OUTDIR]
db-sync-epoch-report.py: error: the following arguments are required: --pg-dbname


$ python3 scripts/db-sync-epoch-report.py --pg-dbname preprod_13.6.0.5_metrics
...
The Conway panels appear automatically when `voting_procedure` / `drep_registration` exist in the Postgres schema; otherwise they're silently omitted. The UTXO count in the summary is included only if `tx_out.consumed_by_tx_id` is populated (same condition as in `db-sync-resource-monitor.py`).
...
A **different** report from `db-sync-epoch-report.py` above: instead of querying Postgres for per-epoch stats, this one assembles the **time-series stats plots** (the figures `db-sync-plot.py` builds from the SQLite stats DBs) into a per-environment **LSM-vs-InMemory** comparison document - automating what used to be a manual "open each HTML, click Plotly's camera button, paste the PNG into a template" chore. SQLite-read-only; never touches Postgres.

For each environment it reads `data/cardano-db-sync/<env>.db` and, for each build present, lays out **CPU & RAM**, **Ingest Metrics**, and **Table Row Counts** on both slot and time axes, then adds comparison sections. Two renderings are produced (pick with `--format`):

- **Markdown + PNGs** (`md`) - a `report.md` referencing auto-rendered `.png` files; paste-able into wikis/Confluence/GitHub. Needs the optional `kaleido` backend (`pip install '.[report]'`).
- **Self-contained interactive HTML** (`html`) - one `report.html` with the live Plotly charts embedded; no extra dependency.
- **`both`** (default).

```bash
# One env, both builds, both formats:
...
python3 scripts/db-sync-stats-report.py --env all --format html \
    --inmemory 13.7.1.0-node-11.0.1 --lsm LSM-13.7.1.0-node-11.0.1 \
    --compare-to 13.6.0.5-genesis
```

`--inmemory` / `--lsm` take the **version token** (the `<ver>` part of the stats label, e.g. `LSM-13.7.1.0-node-11.0.1`); pass at least one. The two builds already coexist in each env's stats DB as distinct version labels, so the LSM build is just the one whose token carries the `LSM-` prefix. The report includes:

1. **InMemory version** - CPU&RAM, Ingest, Tables, each slot + time axis.
2. **LSM version** - same.
3. **LSM vs InMemory Comparison** - the two builds overlaid (headline metrics, **slot axis**).
4. **This vs Previous** - only with `--compare-to <token>`: current builds overlaid with the previous version (slot axis).
...
**Detailed docs:** [docs/10 - Generating reports](docs/10-generating-reports.md) is the full plain-language guide (the version-token naming model and the common quoting mistake, worked examples, formats, troubleshooting); [docs/11 - Report generator internals](docs/11-report-generator-internals.md) is the step-by-step code/architecture primer.

# `node-stats-report.py`

The **cardano-node sibling** of `db-sync-stats-report.py` above, built on the same machinery (`scripts/_report.py`). Instead of the db-sync SQLite stats DB it reads the **cardano-node** one and assembles the node time-series plots (the figures `node-plot.py` builds) into a per-environment **InMemory-vs-LSM** comparison document. SQLite-read-only; never touches Postgres.

For each environment it reads `data/cardano-node/<env>.db` and, for each build present, lays out **CPU & RAM**, **Ingest Metrics**, **On-disk Size**, and **RTS** plots, then adds comparison sections. Two renderings are produced (pick with `--format`): **Markdown + PNGs** (`md`, needs the optional `kaleido` backend - `pip install '.[report]'`), **self-contained interactive HTML** (`html`, no extra dependency), or **`both`** (default).
...
# All envs, add a previous-version comparison, HTML only (no kaleido needed):
python3 scripts/node-stats-report.py --env all --format html \
    --inmemory 11.0.1 --lsm LSM-11.0.1 --compare-to 10.1.4
```

`--inmemory` / `--lsm` take the **version token** (the `<ver>` part of the stats label, e.g. `LSM-11.0.1`); pass at least one. The two builds already coexist in each env's stats DB as distinct version labels, so the LSM build is just the one whose token carries the `LSM-` prefix. The report includes:

1. **InMemory version** - CPU & RAM, Ingest, On-disk Size, RTS.
2. **LSM version** - same.
3. **LSM vs InMemory Comparison** - the two builds overlaid (CPU & RAM, Ingest, On-disk Size).
4. **This vs Previous** - only with `--compare-to <token>`.

Two cardano-node specifics differ from the db-sync report:

- **Ingest is plotted on an epoch axis** (sync time by era + per-epoch duration), so it renders once per build rather than on both slot and time axes. CPU & RAM, Disk, and RTS render on both axes; comparisons align by chain position (slot for CPU & RAM / Disk, epoch for Ingest).
- **Disk and RTS are optional.** They come from the separate `node-db-size-monitor.py` / `node-rts-monitor.py` collectors; a DB that never ran them simply omits those plots (the rest of the report is still produced). RTS is intentionally left out of the comparison overlays (too dense) but appears in each build's own section.

Output goes to `reports/cardano-node/<env>/` (`report.md` + PNGs and/or `report.html`). `--scale`, `--sqlite-db`, and `--html-max-points` behave exactly as in `db-sync-stats-report.py`. Because the RTS collector samples many metrics, node `report.html` files can be larger than db-sync ones - reach for `--html-max-points 4000` if needed.

**Detailed docs:** [docs/13 - Generating reports (cardano-node)](docs/13-node-report-generation.md) is the full plain-language guide; [docs/11 - Report generator internals](docs/11-report-generator-internals.md) (shared with db-sync) is the code/architecture primer.

# `stats-summary.py`

A quick **text summary** of a run, straight from the SQLite stats DB - the "what happened, at a glance" tool. Cross-role (auto-detects node vs db-sync from the DB), SQLite-read-only, no extra dependencies. It runs the queries from [docs/12 - Useful queries](docs/12-useful-queries.md) and converts units to binary GiB for you. (Distinct from the two report tools above: `db-sync-epoch-report.py` reads Postgres; `db-sync-stats-report.py` produces plot PNGs/HTML; this one prints text.)

```bash
# Overview of every run in an environment's DB:
python3 scripts/stats-summary.py --env preprod --role db-sync

# Deep dive on one build (peak RAM, disk/lsm, ingest, top tables, etc.):
python3 scripts/stats-summary.py --env preprod --role db-sync --version 13.7.1.0-node-11.0.1
python3 scripts/stats-summary.py --env mainnet --role node --version LSM-11.0.1
```

Without `--version` it prints a one-row-per-version **overview** (samples, duration, peak RAM, peak CPU, and - when disk sizes were collected - peak disk, plus peak `lsm/` for LSM builds); with `--version <token>` it prints a detailed multi-section breakdown (the `lsm/` figures appear only when that build actually used LSM). `--role` is only needed when both a node and a db-sync DB exist for the same env (otherwise it's auto-detected); `--sqlite-db <path>` points at a specific file; `--top N` controls how many tables the db-sync detail view lists.
...
- **One writer at a time** is still the rule (SQLite is single-writer regardless of mode). This matters here because **more than one collector can write the same env DB at once**: for a node run you might have `node-resource-monitor.py`, `node-rts-monitor.py`, and `node-db-size-monitor.py` all appending to `data/cardano-node/<env>.db`; for db-sync, `db-sync-resource-monitor.py` and `db-sync-ledger-size-monitor.py` share `data/cardano-db-sync/<env>.db`. WAL serializes those writers, so when two collide one has to wait for the lock.

For that waiting to be safe, the collectors open their write connections through `_common.connect_writer()`, which sets a **30-second busy timeout** (`WRITE_TIMEOUT_SEC`) - far longer than SQLite's 5-second default - so a blocked writer waits for the lock instead of immediately raising `database is locked`. As a final guard, each collector's sample loop catches a residual `OperationalError` and **drops that one sample with a warning** rather than crashing a multi-day run. (See [docs/05 - Multiple writers and `busy_timeout`](docs/05-database-internals.md#multiple-writers-and-busy_timeout) for the mechanics, and the [2026-06-08 post-mortem](docs/postmortems/2026-06-08-database-is-locked-collector-crash.md) for the incident that prompted it.)
...
> For read-only day-to-day queries against these SQLite stats DBs (peak RAM, on-disk / `lsm/` size, CPU, row counts, RTS heap, LSM-vs-InMemory comparisons - units pre-converted to GiB), see [docs/12 - Useful queries](docs/12-useful-queries.md).
...
A db-sync DB has five tables that carry the `version` column (`memory_metrics`, `cpu_metrics`, `db_sync_version`, `ingest_metrics`, `table_rowcounts`); a node DB has six (`memory_metrics`, `cpu_metrics`, `node_version`, `node_ingest_metrics`, `disk_metrics`, `rts_metrics`). The authoritative list is `VERSION_KEYED_TABLES` in `scripts/_common.py` (enforced by `tests/test_version_tables_registry.py`). Every recipe below must touch each of them, otherwise stale rows under the old label will reappear in plots and reports.
...
The `cardano-node` DBs in `data/cardano-node/` use the same `memory_metrics` and `cpu_metrics` tables, plus a `node_version` table (instead of `db_sync_version`), a `node_ingest_metrics` table (the equivalent of db-sync's `ingest_metrics`, holding per-sample tip/slot/era data), and - when the optional collectors are run - `disk_metrics` (from `node-db-size-monitor.py`) and `rts_metrics` (from `node-rts-monitor.py`). Version strings look like `cardano-node 10.1.4 preprod`. There is no `table_rowcounts` analogue on the node side. **Prefer `scripts/rename-version.py`** for renames - it touches all of these from the single `VERSION_KEYED_TABLES` registry, so the hand recipe below can't drift out of date (it did: `disk_metrics`/`rts_metrics` were once omitted here, stranding those rows).

Delete (all six version-keyed tables):
...
DELETE FROM disk_metrics        WHERE version = 'cardano-node 10.1.4 preprod';
DELETE FROM rts_metrics         WHERE version = 'cardano-node 10.1.4 preprod';
```

Rename (same six tables):
...
UPDATE disk_metrics        SET version = 'cardano-node 10.1.4 preprod' WHERE version = 'cardano-node 10.1.4';
UPDATE rts_metrics         SET version = 'cardano-node 10.1.4 preprod' WHERE version = 'cardano-node 10.1.4';
EOF
```
...
python3 scripts/node-resource-monitor.py --env mainnet --node-ver 11.0.1-lsm \
  --match-arg lsm

# Or two db-syncs by --state-dir path:
python3 scripts/db-sync-resource-monitor.py --env mainnet --db-sync-ver 13.7.1.0-lsm \
...
python3 scripts/db-sync-resource-monitor.py --env mainnet --db-sync-ver 13.7.1.0-lsm \
  ...
  > logs/db-sync-resource-monitor-lsm.log 2>&1 &

# Terminal 2
python3 scripts/db-sync-resource-monitor.py --env mainnet --db-sync-ver 13.7.1.0-inmem \
  --pg-dbname mainnet_inmem --match-arg inmem \
  > logs/db-sync-resource-monitor-inmem.log 2>&1 &
```
...
You restarted the monitor while a sync was in progress. The two sampling sessions are connected by a straight line across the gap. Current builds insert a NaN marker once a gap exceeds 5× the series' median sample interval (≈50s for the 10s collectors, ≈300s for the 60s disk collector) so the line breaks cleanly; if you see a cliff in a current plot, the gap may be just under threshold - pass `--x-axis time` to make it more obvious.
...
```bash
python3 scripts/node-resource-monitor.py --env preprod --node-ver 11.0.1 --interval 1
```
