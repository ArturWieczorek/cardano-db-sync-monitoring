...
This is why our era classification (in `db-sync-epoch-report.py` and the node ingest plot) groups epochs by `protocol_major`: it's the chain's authoritative answer to "what era was this epoch in?"
