...
If you want to see *smoothed* rates, average over multiple intervals - a 5-sample (50-second) rolling mean filters out mild burst noise while preserving the medium-scale trend. Be aware this is a weak tool when the spikes are large and isolated rather than fast wiggle: measured on mainnet, a 5-sample rolling mean cut the block-rate spread by only ~1% (it just smears each spike sideways), versus ~13% for the gentler tx-rate. For the deeper treatment - how to *measure* spikiness (coefficient of variation, percentiles, jumpiness), why much of the block-rate spread is real signal (the Byron bulk-load is ~50x faster than the modern chain), and which display tricks actually help (a logarithmic y-axis and a rolling median, not a short mean) - see [09 - Reading and smoothing bursty rates](09-reading-and-smoothing-bursty-rates.md).
