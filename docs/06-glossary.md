...
**Gap break** - a NaN-valued marker row inserted between consecutive samples whose wall-clock gap exceeds 5× that series' own median sample interval (adaptive, so a 10s collector breaks near 50s and the 60s disk collector near 300s; falls back to 50s when unmeasurable). Forces plotly to break the line at that point so visualization doesn't fabricate values across sampling gaps.

**`HOT_TABLES`** - constant in `db-sync-resource-monitor.py` listing the postgres tables we sample row counts of every interval. Currently: `block`, `tx`, `tx_out`, `ma_tx_out`, `ma_tx_mint`, `multi_asset`, `datum`, `redeemer`, `script`.
...
**`run_label`** - the full label written into the `version` column of every row. Format: `cardano-db-sync <ver> <env>` or `cardano-node <ver> <env>`, where `<ver>` is the `--db-sync-ver` / `--node-ver` you passed. It is the join key across every table and across collectors, so all collectors for one run must use the *same* `--node-ver`: a typo (e.g. passing the env-dir name `LSM-mainnet` instead of the version `11.0.1`) shards the data under a second label and the plot won't group them. The collectors now warn at startup when a new label appears alongside existing ones; to merge an already-sharded label use `scripts/rename-version.py`.
...
**`wait_for_schema`** - db-sync-resource-monitor method that polls until the `block` table exists. Prevents the "relation does not exist" noise during the first few iterations of a fresh sync.
