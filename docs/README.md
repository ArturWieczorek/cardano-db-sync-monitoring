...
| 15 | [Tracking and benchmarking rollbacks](15-tracking-and-benchmarking-rollbacks.md) | Task-oriented how-to for the two rollback tools, per case: watch a live db-sync (metrics / +log), see and plot what was captured, benchmark one version on demand, compare two versions fairly, and confirm data equivalence. Plus the learned-the-hard-way gotchas. |
| 16 | [Compare rollback performance, step by step](16-compare-rollback-performance-step-by-step.md) | Detailed beginner walkthrough of the headline task (is a new version's rollback slower?): the problem and why a fair comparison is hard, the snapshot/normalize approach and why it's fair, what is and isn't measured, then exact copy-paste steps with "what you should see", interpretation, and fairness caveats. |
| 17 | [Reproducing issue #2083 (lengthy rollbacks)](17-reproducing-issue-2083-lengthy-rollbacks.md) | The real-world regression these tools exist for: what the rollback query-planner bug was, which versions are affected/fixed, and three methods to reproduce/confirm/track it (a one-line `EXPLAIN` root-cause check with a worked mainnet example, the end-to-end pre-fix-vs-fixed benchmark, and the live-monitor diagnostic). |
