...
def plot_epoch_stats(per_db: dict[str, tuple[pd.DataFrame, pd.DataFrame]], outdir: str) -> str:
    ...
    if have_conway:
        add_row([{"colspan": 2}, None], "Conway: Voting Procedures & Drep Registrations")
    ...
    if have_era:
        for dbname in dbnames:
            ...
            fig.add_trace(
                go.Bar(
                    x=era_df["era"],
                    y=era_df["total_sync_secs"],
                    name=dbname if is_compare else "era_total",
                    text=None
                    if is_compare
                    else [f"{v:.0f}s ({p:.1f}%)" for v, p in zip(era_df["total_sync_secs"], era_df["pct_of_total"])],
                    textposition="auto",
                ),
                row=row,
                col=1,
    ...
    for dbname in dbnames:
        ...
        e = df.epoch_no
        r = row
        fig.add_trace(go.Scatter(x=e, y=df["sync_secs"], mode="lines", name=label("sync_secs", dbname)), row=r, col=1)
        r += 1
        fig.add_trace(
            go.Scatter(x=e, y=df["block_count"], mode="lines", name=label("block_count", dbname)), row=r, col=1
        )
        fig.add_trace(go.Scatter(x=e, y=df["tx_count"], mode="lines", name=label("tx_count", dbname)), row=r, col=2)
        r += 1
        fig.add_trace(go.Scatter(x=e, y=df["total_fees"], mode="lines", name=label("total_fees", dbname)), row=r, col=1)
        fig.add_trace(
            go.Scatter(x=e, y=df["total_output"], mode="lines", name=label("total_output", dbname)), row=r, col=2
        )
        r += 1
        fig.add_trace(
            go.Scatter(x=e, y=df["avg_tx_size"], mode="lines", name=label("avg_tx_size", dbname)), row=r, col=1
        )
        if have_p95:
            fig.add_trace(
                go.Scatter(
                    x=e, y=df["p95_tx_size"], mode="lines", name=label("p95_tx_size", dbname), line=dict(dash="dot")
                ),
                row=r,
                col=1,
            )
        fig.add_trace(
            go.Scatter(x=e, y=df["sum_tx_size"], mode="lines", name=label("sum_tx_size", dbname)), row=r, col=2
        )
        r += 1
        if have_plutus:
            fig.add_trace(
                go.Scatter(x=e, y=df["plutus_ratio"], mode="lines", name=label("plutus_ratio", dbname)), row=r, col=1
            )
            fig.add_trace(
                go.Scatter(x=e, y=df["ma_mint_count"], mode="lines", name=label("ma_mint_count", dbname)), row=r, col=2
            )
        else:
            fig.add_trace(
                go.Scatter(x=e, y=df["ma_mint_count"], mode="lines", name=label("ma_mint_count", dbname)), row=r, col=1
            )
        r += 1
        if have_cumulative:
            fig.add_trace(
                go.Scatter(x=e, y=df["cumulative_assets"], mode="lines", name=label("cumulative_assets", dbname)),
                row=r,
                col=1,
            )
            r += 1
        fig.add_trace(
            go.Scatter(x=e, y=df["reward_count"], mode="lines", name=label("reward_count", dbname)), row=r, col=1
        )
        fig.add_trace(
            go.Scatter(x=e, y=df["stake_count"], mode="lines", name=label("stake_count", dbname)), row=r, col=2
        )
        r += 1
        if have_conway:
            if have_voting:
                fig.add_trace(
                    go.Scatter(x=e, y=df["voting_count"], mode="lines", name=label("voting_procedures", dbname)),
                    row=r,
                    col=1,
                )
            if have_drep:
                fig.add_trace(
                    go.Scatter(x=e, y=df["drep_reg_count"], mode="lines", name=label("drep_registrations", dbname)),
                    row=r,
                    col=1,
                )
...
def _write_summary_body(f: Any, summary: dict[str, Any], dbname: str) -> None:
    ...
    if era_df is not None and not era_df.empty:
        ...
        for _, r in era_df.iterrows():
            ...
            f.write(
                f"  {r['era']:<10}{int(r['epochs']):>8}  {rng:<14}"
    ...
    if summary["utxo_tracking"]:
        ...
        f.write(
            "  UTXO set size:         tracking disabled "
...
def write_summary_report_comparison(
    ...
    with open(filename, "w") as f:
        ...
        f.write(
            line(
                "Distinct MA assets (est.)",
                f"{sa['total_distinct_assets']:,}",
                f"{sb['total_distinct_assets']:,}",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--pg-port", default=None, type=lambda v: int(v), help="Postgres port (default: PGPORT env var, then 5432)"
    )
