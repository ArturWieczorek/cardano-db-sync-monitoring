...
data/cardano-db-sync/<env>.db under the same version label db-sync-monitor.py uses.
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--db-sync-ver", required=True,
                   help="Label tagging this run (e.g. LSM-13.7.1.0). Match the --db-sync-ver you "
                        "gave db-sync-monitor so the disk series joins the rest of the run.")
    ...
    p.add_argument("--sqlite-db", default=None,
                   help="Override the SQLite DB path (default: data/cardano-db-sync/<env>.db).")
