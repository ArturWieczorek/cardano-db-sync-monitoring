...
node-monitor.py uses — so the RTS curves join the rest of the run.

Why a separate script (and node-monitor.py left untouched):
  - It adds an HTTP scrape per interval. That's isolated here behind a timeout
    ...
    psutil/tip sampling node-monitor.py does. Zero risk to the running monitor.
...
DEFAULT_INCLUDE: tuple[str, ...] = (
    "rts", "gc", "alloc", "heap", "live", "mempool",
)
...
class NodeRtsMonitor:
    def __init__(self, env: str, node_ver: str, url: str, includes: list[str],
                 interval: float, timeout: float, emit_json: bool,
    ...
    def emit(self, ts: str, slot: int | None, selected: dict[str, float]) -> None:
        if self.emit_json:
            print(json.dumps({
                "ts": ts, "env": self.env, "label": self.node_ver,
                "version": self.run_label, "slot_no": slot, "metrics": selected,
            }))
        else:
    ...
    def run(self) -> None:
        ...
        while self.running:
            ...
            if result is None:
                if not warned_empty:
                    warn("Endpoint reachable but no metrics matched the allowlist — is the node "
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--env", required=True, choices=["mainnet", "preprod", "preview"],
                   help="Environment name (selects data/cardano-node/<env>.db and the version label)")
    p.add_argument("--node-ver", required=True,
                   help="Label tagging this run (e.g. LSM-11.0.1). Match the --node-ver you gave "
                        "node-monitor so the RTS series joins the rest of the run.")
    p.add_argument("--prometheus-url", default=DEFAULT_PROMETHEUS_URL,
    ...
    p.add_argument("--interval", type=float, default=10.0,
                   help="Sampling interval in seconds (default: 10 — the scrape is light).")
    ...
    p.add_argument("--sqlite-db", default=None,
                   help="Override the SQLite DB path (default: data/cardano-node/<env>.db).")
    return p.parse_args()
