...
def _load(script_name: str, module_alias: str):
    spec = importlib.util.spec_from_file_location(module_alias, SCRIPTS_DIR / script_name)
...
@pytest.fixture(params=[("db_sync", db_sync_plot), ("node", node_plot)], ids=["db-sync-plot", "node-plot"])
...
class TestOutPath:
    ...
    def test_env_is_first_word_of_filename(
        ...
        convention used by db-sync-epoch-report's outputs."""
        _label, mod = out_path_fn
        for env in ("mainnet", "preprod", "preview"):
            ...
            name = Path(path).name
            assert name.startswith(f"{env}_"), f"filename must start with the env; got: {name!r}"
    ...
    def test_two_versions_env_appears_once_not_per_version(
        ...
        assert name.count("preprod") == 1, f"env must appear exactly once; got: {name!r}"
    ...
    def test_axis_only_two_values_slot_or_time(
        ...
        assert slot_path != time_path, (
            "slot and time outputs must not collide - that's why the axis tag exists. Got: " + repr(slot_path)
        )
