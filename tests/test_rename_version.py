...
def _load_rename_module():
    ...
    spec = importlib.util.spec_from_file_location("rename_version_module", SCRIPTS_DIR / "rename-version.py")
...
def _make_node_db(path: Path, rows_per_version: dict[str, int]) -> None:
    """Create a minimal node stats DB with all six version-keyed tables."""
    conn = sqlite3.connect(str(path))
    try:
        conn.execute("CREATE TABLE memory_metrics      (version TEXT, payload TEXT)")
        conn.execute("CREATE TABLE cpu_metrics         (version TEXT, payload TEXT)")
        ...
        conn.execute("CREATE TABLE disk_metrics        (version TEXT, payload TEXT)")
        conn.execute("CREATE TABLE rts_metrics         (version TEXT, payload TEXT)")
        for v, n in rows_per_version.items():
            for t in ("memory_metrics", "cpu_metrics", "node_ingest_metrics", "disk_metrics", "rts_metrics"):
                conn.executemany(
                    f"INSERT INTO {t} (version, payload) VALUES (?, ?)",
                    [(v, f"p{i}") for i in range(n)],
                )
...
def _total_rows_for_version(db: Path, tables: tuple[str, ...], version: str) -> int:
    ...
    try:
        total = 0
        for t in tables:
            (n,) = conn.execute(f"SELECT COUNT(*) FROM {t} WHERE version = ?", (version,)).fetchone()
            total += n
        return total
    finally:
        conn.close()
...
class TestCountFor:
    def test_counts_per_table(self, tmp_path: Path) -> None:
        ...
        with sqlite3.connect(str(db)) as conn:
            counts = rename_version.count_for(conn, rename_version.VERSION_TABLES["db-sync"], "v1")
        assert counts == {
            "memory_metrics": 3,
            "cpu_metrics": 3,
            "db_sync_version": 3,
...
class TestRenameInDb:
    ...
    def test_node_renames_all_six_tables(self, tmp_path: Path) -> None:
        db = tmp_path / "node.db"
        ...
        # disk_metrics and rts_metrics must be covered too (the bug that left a
        # mislabeled RTS series stranded was this list omitting them).
        assert {"disk_metrics", "rts_metrics"} <= set(tables)
        assert _total_rows_for_version(db, tables, "old") == 0
...
class TestRenameInDbExtra:
    ...
    def test_skips_missing_table_during_update(self, tmp_path: Path) -> None:
        ...
        present = ("memory_metrics", "cpu_metrics", "db_sync_version", "ingest_metrics")
...
class TestResolvePath:
    ...
    def test_explicit_path_overrides(self) -> None:
        ns = self._ns(path="/tmp/anywhere.db", from_version="cardano-db-sync 1 preprod", env="preprod")
    ...
    def test_db_sync_prefix_infers_role(self) -> None:
        ...
        assert rename_version.resolve_path(ns) == (rename_version.ROLE_DIR["db-sync"] / "preprod.db")

    def test_node_prefix_infers_role(self) -> None:
        ns = self._ns(
            env="mainnet", from_version="cardano-node 11.0.1 mainnet", to_version="cardano-node 11.0.1 mainnet-renamed"
        )
        assert rename_version.resolve_path(ns) == (rename_version.ROLE_DIR["node"] / "mainnet.db")
    ...
    def test_explicit_role_overrides_prefix_inference(self) -> None:
        ...
        assert rename_version.resolve_path(ns) == (rename_version.ROLE_DIR["node"] / "preview.db")

    def test_unknown_prefix_raises(self) -> None:
        ns = self._ns(env="preprod", from_version="not-a-known-prefix", to_version="not-a-known-prefix-renamed")
...
class TestMainCli:
    ...
    def test_default_takes_backup_and_renames(
        ...
        self._argv(monkeypatch, "--path", str(db), "--from-version", "old", "--to-version", "new")
    ...
    def test_dry_run_skips_both_backup_and_writes(
        ...
        self._argv(monkeypatch, "--path", str(db), "--from-version", "old", "--to-version", "new", "--dry-run")
        rc = rename_version.main()
        assert rc == 0
    ...
    def test_no_backup_flag_skips_backup_but_writes(
        ...
        self._argv(monkeypatch, "--path", str(db), "--from-version", "old", "--to-version", "new", "--no-backup")
        rc = rename_version.main()
        assert rc == 0
        assert list(tmp_path.glob("test.db.bak-*")) == []
        tables = rename_version.VERSION_TABLES["db-sync"]
    ...
    def test_missing_db_returns_1(
        ...
        self._argv(
            monkeypatch, "--path", str(tmp_path / "does-not-exist.db"), "--from-version", "old", "--to-version", "new"
        )
        rc = rename_version.main()
        assert rc == 1
    ...
    def test_identical_versions_argparse_error(
        ...
        self._argv(monkeypatch, "--path", "/tmp/anything.db", "--from-version", "same", "--to-version", "same")
    ...
    def test_neither_path_nor_env_rejected(
        ...
        self._argv(monkeypatch, "--from-version", "a", "--to-version", "b")
        with pytest.raises(SystemExit) as ei:
            rename_version.main()
        assert ei.value.code == 2
